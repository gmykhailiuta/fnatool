...
def fft_filter(time, signal, result, freq_limit = (0.004, 0.04)):
  ...
  spec_filt_abs *= 2 # we cut off half of spectra - needs to be compensated
  spec_abs *= 2
...
def bandPass_(T, V, fname):
  ...
  for i in range(n):  # frequency cutting
    if 0.0033 < abs(ffs[i]) and abs(ffs[i]) < 0.04:
    #if abs(ffs[i]) < 0.0033:
    #if abs(ffs[i]) < 0.4:
      P[i] = I[i]
...
def plot_beta_std(results):
  x = [x['std'] for x in results]
  y = [y['beta'] for y in results]
  pl.figure(figsize=(6, 6), facecolor='white')
  pl.title(r'%(record)s: $\beta/\sigma$' % results)
  pl.plot(x,y,'.r')
  pl.ylim(0,2)
  pl.xlim(0,140)
  pl.axhline(y=1,color='b')
...
def plot_beta(freq, fft, aprox_y, result):
  pl.figure(figsize=(6, 6),facecolor='white')
  pl.plot(freq,fft,'b')
...
def process_signal(ann_file_name, diagnosis=None):
    ...
    #plot_beta_cv(results, "cv_%s.png" % (record, ))

if __name__ == '__main__':
  #signals = ['16273 atr', '16272 atr',  'chf06 ecg', 'chf07 ecg', '16420 atr', 's20051 atr']
  #signals = ['s20051 atr', 's20411 atr']
  #signals = ['16265.atr  16272.atr  16273.atr  16420.atr  16483.atr  16539.atr  16773.atr  16786.atr  16795.atr']
